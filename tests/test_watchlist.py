...
Follows the same fixture and assertion structure as tests/test_collection.py.
"""
...
def test_get_watchlist_returns_newest_first(app, sample_user):
    ...
    This is the additional edge-case test beyond what was requested in Comment 3.
    It verifies the sort order matches the collection pattern — which is important
    because the original code sorted alphabetically, making watchlist and collection
    behave differently for no good reason.
    """
    with app.app_context():
...
def test_add_to_watchlist_respects_public_false(app, sample_user, sample_film):
    """
    When public=False is passed explicitly, the entry should be private.
    """
    with app.app_context():
...
def test_add_to_watchlist_defaults_to_public(app, sample_user, sample_film):
    """
    When public is not specified, the entry should default to public=True.
    """
    with app.app_context():
        entry = add_to_watchlist(user_id=sample_user, film_id=sample_film)
